...
impl RenderDistance {
    ...
    pub fn iterate_around_player(self, player_pos: (f64, f64, f64)) -> impl Iterator<Item = ChunkPos> {
    ...
    pub fn is_chunk_visible(self, player_pos: (f64, f64, f64), chunk_pos: ChunkPos) -> bool {
        let player_chunk = BlockPos::from(player_pos).containing_chunk_pos();
...
impl Iterator for RenderDistanceIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.i > self.render_distance.x_max as i64 {
            ...
            self.k += 1;
            Some((self.i + self.player_chunk.px, self.j + self.player_chunk.py, self.k + self.player_chunk.pz - 1).into())
        }
    }
}
