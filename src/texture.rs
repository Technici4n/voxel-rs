...
use texture_packer::{TexturePackerConfig, TexturePacker};
...
pub fn load_textures<F, R>(factory: &mut F, textures: Vec<PathBuf>) -> Result<(ShaderResourceView<R, [f32; 4]>, Vec<TextureRect>)>
where
    F: gfx::Factory<R>,
    R: gfx::Resources,
{
    ...
    for (i, path) in textures.iter().enumerate() {
        packer.pack_own(format!("{}", i), ImageImporter::import_from_file(path).expect("Failed to read texture to pack"));
    }

    let mut texture_buffer = ImageBuffer::new(MAX_TEXTURE_SIZE, MAX_TEXTURE_SIZE);
    texture_buffer.copy_from(&ImageExporter::export(&packer).expect("Failed to export texture from packer"), 0, 0);
    ...
    Ok((
        texture_view,
        (0..textures.len()).map(|i| {
            let frame = packer.get_frame(&format!("{}", i)).expect("Texture packer frame key doesn't exist").frame;
            TextureRect {
