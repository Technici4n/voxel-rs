...
impl World {
    ...
    pub fn create_adj_chunk_occl(&self, ix: i64, iy: i64, iz: i64) -> AdjChunkOccl {
        ...
        for i in 0..6 {
            faces[i] = match self.get_chunk(ix + da[i][0], iy + da[i][1], iz + da[i][2]) {
                Some(chunk) => {
        ...
        for i in 0..12 {
            edges[i] = match self.get_chunk(ix + de[i][0], iy + de[i][1], iz + de[i][2]) {
                Some(chunk) => {
        ...
        for i in 0..8 {
            coins[i] = match self.get_chunk(ix + dc[i][0], iy + dc[i][1], iz + dc[i][2]) {
                None => false,
                Some(chunk) => {
