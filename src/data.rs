...
use crate::{
    block::{Block, BlockData, BlockMeshData, BlockType, mesh::BlockMesh},
    registry::Registry,
};
use log::info;
use std::fs;
...
pub fn load_data(factory: &mut gfx_device_gl::Factory, data_directory: PathBuf) -> Result<Data> {
    ...
    info!("Loading textures from directory {}", textures_directory.display());
    for dir_entry in fs::read_dir(textures_directory).context("couldn't read textures directory")? {
        ...
        if dir_entry.file_type().context("failed to get file type")?.is_file() {
            let file_path = dir_entry.path();

            texture_registry.register(file_path.file_stem().context("failed to get file stem")?.to_str().unwrap().to_owned(), ());
    ...
    info!("Loading blocks from directory {}", blocks_directory.display());
    for dir_entry in fs::read_dir(blocks_directory).context("couldn't read block directory")? {
        ...
        if dir_entry.file_type().context("failed to get file type")?.is_file() {
            ...
            match file_path.extension() {
                ...
                Some(ext) => {
                    if ext == "ron" {
                        let mut file = fs::File::open(file_path.clone()).context("couldn't open .ron block file")?;
                        let mut buffer = String::new();
                        file.read_to_string(&mut buffer);
                        block_datas.push(
                            (file_path.file_stem().context("failed to get file stem")?.to_str().unwrap().to_owned(), ron::de::from_str(&buffer).context("failed to parse .ron block file")?)
                        );
                    } else {
    ...
    // Add air
    blocks.register("air".to_owned(), Block { name: "air".to_owned(), block_type: BlockType::Air });
